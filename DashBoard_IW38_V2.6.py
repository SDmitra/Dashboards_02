def main():
    ...
    current_date_normal = current_date.strftime('%y.%m.%d')
    ...
    with cols1[0]:
        ...
        if selected_sp == 'Все' or selected_sp == 'Несколько СП':
            grouped_data_date1 = grouped_data_date1.groupby('org_sp_kod')['number_1'].sum().reset_index()
        elif selected_sp != 'Все' and selected_sp != 'Несколько СП':
            grouped_data_date1 = grouped_data_date1.groupby('gp_kod')['number_1'].sum().reset_index()
        ...
        if selected_sp == 'Все' or selected_sp == 'Несколько СП':
            ...
            grouped_data_date2 = grouped_data_date2.groupby('gp_kod')['number_1'].sum().reset_index()
        ...
        if selected_sp == 'Все' or selected_sp == 'Несколько СП':
            ...
            merged_data = pd.merge(grouped_data_date1, grouped_data_date2, on='gp_kod', suffixes=('_date1', '_date2'), how='outer') 
        ...
        if selected_sp == 'Все' or selected_sp == 'Несколько СП':
            ...
            # Создаем столбец с разницей между суммами для двух дат
            merged_data['difference'] = merged_data['number_1_date2'] - merged_data['number_1_date1']
            merged_data = pd.merge(merged_data,gp, on = 'gp_kod', how = 'right')
            columns_to_keep = ['gp_kod', 'number_1_date1', 'number_1_date2', 'difference']
            # Отображаем полученную сводную таблицу с разницей между датами
            merged_data = merged_data[columns_to_keep]
    ...
    merged_data.dropna(subset=['difference'], inplace=True)
    ...
    st.sidebar.download_button(
        label="Экспорт в CSV",
        data=csv,
        file_name=str(current_date_normal)+"_"+str(selected_product),
        mime='text/csv',
    )
