def main():
    ...
    merged_df = pd.merge(ar, org, on='org_sp_kod', how='left')
    merged_df = pd.merge(merged_df, gp, on='org_sp_kod', how='left')
    ...
    def check_order_status(row):
        if row['date_1'] < row['date_dataframe'] and row['text_2'] in zakr_status:
            ...
            return "2. Заказ просрочен"
        elif row['date_1'] > row['date_dataframe'] and row['text_2'] not in zakr_status:
            return "4. Заказ актуален"
        else:
            return "3. Заказ закрыт в будущем"

    # Добавление нового столбца с помощью apply и функции check_order_status
    ar['order_status'] = ar.apply(check_order_status, axis=1)
    #ГРАФИК
    grouped_data_zkz = ar.groupby(['date_dataframe', 'org_sp_kod','order_status'])['number_2'].sum().reset_index()
    fig2 = px.bar(grouped_data_zkz, x='org_sp_kod', y='number_2', color='order_status',
                            title='График количества типов заказов по СП', labels={'org_sp_kod': 'Завод', 'order_status': 'Количество заказов'})
    fig2.update_layout(
        xaxis_title='СП',
        yaxis_title='Заказы',
    ...
    def metric_08_01(dates_chooses_00, selected_bes, selected_sps, selected_gps, selected_subgps):
        ...
        if 'Все' in selected_bes:
            ...
            grouped_data = merged_df.groupby([date_dataframe, 
        ...
        if 'Все' not in selected_bes:
            ...
            grouped_data = merged_df[merged_df[org_be_fullname_rus].isin(selected_bes)]
            ...
            if 'Все' not in selected_sps:
                ...
                grouped_data = merged_df[merged_df[org_sp_rank_mixname_rus].isin(selected_sps)]
                grouped_data = grouped_data.groupby([date_dataframe, 
                                                org_sp_rank_mixname_rus])['number_1'].sum().reset_index()
                level = org_sp_rank_mixname_rus
                ...
                if 'Все' not in selected_gps:
                    ...
                    grouped_data = merged_df[merged_df[gp_group_rank_shortname_rus].isin(selected_gps)]
                    grouped_data = grouped_data.groupby([date_dataframe, 
                    ...
                    if 'Все' not in selected_subgps:
                        ...
                        df_main_pivot_table1 = pd.pivot_table(df_main,
                                                            ...
                                                            columns='date_dataframe',
                                                            aggfunc='sum')
                        grouped_data = merged_df[merged_df[gp_subgroup_rank_shortname_rus].isin(selected_subgps)]
                        grouped_data = grouped_data.groupby([date_dataframe, 
        ...
        if dates_chooses_00 != []:
            ...
            metric_main_01(df_main_pivot_table1_filtered, dates_chooses_00,grouped_data,level,level_name,df_main)
        if dates_chooses_00 == []:
    ...
    def metric_main_01(df_main_pivot_table1, dates_chooses_00,grouped_data,level,level_name,df_main):
        ...
        df_styled = df_main_pivot_table1_with_difference.style.applymap(lambda x: 'background-color: #ff6666' if x > 0 else 'background-color: #99ff99', subset=[differences.name])
